...
#for converting between Cartesian and polar coordinates. 
...
SPAR_smooth = function(sample_data,norm_choice="L1",thresh_prob,k,k_shape=NULL,pred_Q=seq(-2,2,length.out=201)){ #wrapper for fitting the SPAR model 
  ...
  return(list(pred_thresh=pred_thresh,pred_para=pred_para,pred_Q=pred_Q,thresh_prob=thresh_prob,exp_quants=exp_quants,polar_data=polar_data,polar_exceedance_data=polar_exceedance_data,k=k,k_shape=k_shape,norm_choice=norm_choice,ald_coeff = m_ald$coefficients[1:(k-1)],gpd_coeff = m_gpd$coefficients))
  
}


#Wrapper for fitting the SPAR modelling framework on the polar scale.
SPAR_smooth_polar = function(sample_data,thresh_prob,k,k_shape=NULL,pred_phi=seq(0,2*pi,length.out=201)){ #wrapper for fitting the SPAR model 
  #sample_data - this denotes the bivariate data centered at (0,0) for which to fit SPAR. Must be an n x 2 matrix, where n denotes number of observations
  #thresh_prob - non-exceedance probability for which to estimate threshold. Must be a probability in (0,1)
  #k - spline basis dimension for threshold and scale functions. Must be a natural number greater that 3
  #k_shape - spline basis dimension for shape functions. Must be equal to a natural number greater that 3, or NULL. Defaults to NULL, corresponding to a constant shape
  #pred_phi - polar angular values at which to evaluate estimated functions. This must be a grid of angles in [0,2*pi]. Defaults to seq(0,2*pi,length.out=201)
  ...
  if(min(pred_phi) < 0 | max(pred_phi)>2*pi | length(pred_phi)<=1){
    stop("pred_phi should be a grid of polar angles in the interval [0,2*pi]")
  }
  
  #We first transform data to polar radial-angular coordinates
  ...
  #Formulation for threshold function. s(Phi, bs='cc',k=k) is a cyclic cubic spline over Phi with basis dimension k
  fmla_ald = paste0("logR ~ s(Phi, bs='cc',k=",k,")")
  ...
  #dataframe of threshold exceedance data. We only save exceedances - we don't need the rest of the radial data
  polar_exceedance_data = data.frame(Phi=polar_data$Phi[positive_exceedances_indicator],R_exc=gpd_thresh_exceedances)
  ...
  if(is.null(k_shape)){
    ...
    #formulation for scale function
    spl=paste0("R_exc ~ s(Phi, bs='cc',k=",k,")")
    ...
    #obtain GPD parameter function estimates for values in pred_phi interval
    pred_para = predict(m_gpd, newdata=data.frame(Phi=pred_phi),type="response")
    
  } else {
    
    #define second angular component that is identical as first. This is so that we can define separate splines for the shape and scale 
    polar_exceedance_data$Phi2 = polar_exceedance_data$Phi
    
    #formulation for both scale and shape. Note that k and k_shape are unlikely to be the same
    fmla_gpd = list(as.formula(paste0("R_exc ~ s(Phi, bs='cc',k=",k,")")), as.formula(paste0("R_exc ~ s(Phi2, bs='cc',k=",k_shape,")"))) 
    
    #fit non-stationary GPD model using evgam 
    m_gpd = evgam(fmla_gpd, data=polar_exceedance_data, family="gpd") 
    
    #obtain GPD parameter function estimates for values in pred_phi interval
    pred_para = predict(m_gpd,type = "response",newdata=data.frame(Phi=pred_phi,Phi2=pred_phi))
    
  }
  
  #obtain threshold function estimate for values in pred_phi interval
  pred_thresh = exp(predict(m_ald, newdata=data.frame(Phi=pred_phi))$location) 
...
#Wrapper for fitting the SPAR modelling framework on the polar scale.
SPAR_angular_density_polar = function(sample_data,pred_phi=seq(0,2*pi,length.out=201),bw=50){ #wrapper for estimating the angular density in the SPAR framework
  #sample_data - this denotes the bivariate data centered at (0,0) for which to fit SPAR. Must be an n x 2 matrix, where n denotes number of observations
  #pred_phi - angular values at which to evaluate estimated functions. This must be a grid of angles in [0,2*pi]. Defaults to seq(0,2*pi,length.out=201)
  #bw - this is the bandwidth parameter for the kernel density estimation of the circular angular density. Should be a positive number. Defaults to 50
  ...
  #estimating the density function using the `circular' R package with a von Mises kernel
  dens_est = density.circular(as.circular(polar_data$Phi,type="angles",units="radians",template="none",modulo="2pi",zero=0,rotation="counter"), bw=bw,kernel="vonmises")
  
  #Making sure we equal 2pi at the endpoint 
  dens_est$x[length(dens_est$x)] = 2*pi
  
  #obtaining the estimated density function.
  f_phi = approxfun(x = dens_est$x,y = dens_est$y) 
  
  #obtain density estimates for values in pred_phi interval
  return(f_phi(pred_phi))
}
...
SPAR_equidensity_contours = function(density_levels,SPAR_GPD,SPAR_ang){
  #density_levels - levels for which equidensity contour is required. Note that this value is bounded above by the fact we do not model data below the threshold function
  ...
  if(norm_choice == "L2"){
    density_levels = (pi/2)*density_levels #adjusting for Jacobian 
  }
...
#Function for estimating equidensity contours from fitted SPAR model on the polar scale.
SPAR_equidensity_contours_polar = function(density_levels,SPAR_GPD,SPAR_ang){
  ...
  #Computing radii values at density value for each fixed angle
  dens_radii = matrix(NA,ncol = length(density_levels),nrow=length(pred_phi))
  for(i in 1:length(density_levels)){
    for(j in 1:length(pred_phi)){
      if(SPAR_GPD$pred_para$shape[j] < 0){
        dens_radii[j,i] = uniroot(joint_dens_root_func,interval = c(SPAR_GPD$pred_thresh[j],(SPAR_GPD$pred_thresh[j] - (SPAR_GPD$pred_para$scale[j])/(SPAR_GPD$pred_para$shape[j]))),u=SPAR_GPD$pred_thresh[j],scale=SPAR_GPD$pred_para$scale[j],shape=SPAR_GPD$pred_para$shape[j],thresh_prob=SPAR_GPD$thresh_prob,f_q=SPAR_ang[j],dens_val=density_levels[i])$root
      } else {
        dens_radii[j,i] = uniroot(joint_dens_root_func,interval = c(SPAR_GPD$pred_thresh[j],50),u=SPAR_GPD$pred_thresh[j],scale=SPAR_GPD$pred_para$scale[j],shape=SPAR_GPD$pred_para$shape[j],thresh_prob=SPAR_GPD$thresh_prob,f_q=SPAR_ang[j],dens_val=density_levels[i])$root
...
SPAR_ret_level_sets = function(ret_period,obs_year,SPAR_GPD){
...
#Function for estimating return level sets from fitted SPAR model on the polar scale.
SPAR_ret_level_sets_polar = function(ret_period,obs_year,SPAR_GPD){
  #ret_period - the return period for evaluating the return level set
  #obs_year - the number of observations per year within the data
  #SPAR_GPD - output from either SPAR_smooth or SPAR_local. Must be a list
...
SPAR_local = function(sample_data,norm_choice="L1",thresh_prob,pred_Q=seq(-2,2,length.out=201),num_neigh=500){ #wrapper for fitting the SPAR model 
  #sample_data - this denotes the bivariate data centered at (0,0) for which to fit SPAR. Must be an n x 2 matrix, where n denotes number of observations
  #norm_choice - this denotes the choice of norm. Must equal either "L1" or "L2". Defaults to "L1"
  #thresh_prob - non-exceedance probability for which to estimate threshold. Must be a probability in (0,1)
  #pred_Q - angular values at which to evaluate estimated functions. This must be a grid of angles in [-2,2]. Defaults to seq(-2,2,length.out=201)
...
#Wrapper for fitting the empirical version of the SPAR model on the polar scale.
SPAR_local_polar = function(sample_data,thresh_prob,pred_phi=seq(0,2*pi,length.out=201),num_neigh=500){ #wrapper for fitting the SPAR model 
  #sample_data - this denotes the bivariate data centered at (0,0) for which to fit SPAR. Must be an n x 2 matrix, where n denotes number of observations
  #thresh_prob - non-exceedance probability for which to estimate threshold. Must be a probability in (0,1)
  #pred_phi - angular values at which to evaluate estimated functions. This must be a grid of angles in [0,2*pi]. Defaults to seq(0,2*pi,length.out=201)
  #num_neigh - the number of neighbours in each local windows. This must be a positive integer greater than 50 for numerical stability. Defaults to 500 
  ...
  #wrapper for obtaining empirical radial windows for each of the reference angles
  SPAR_empirical_windows = function(ref_phi,polar_data,num_neigh){
    #ref_phi - reference angle 
    #polar_data - angular radial dataframe 
    #num_neigh - number of neighbours for each local angular windows
    
    #finding differences between all observed angles and the reference angle. This metric is adjusted to account for periodicity in angles
    diff = apply(cbind(2*pi - abs(polar_data$Phi - ref_phi),abs(polar_data$Phi - ref_phi)),1,min)
    
    #computing indices of num_neigh closest observed angles to ref_phi
    eps.Phi = sort(diff)[num_neigh]
    nei.Phi = which(diff <= eps.Phi)
    
    #returning radial component at angles within local window
    return(polar_data$R[nei.Phi])
  }
  ...
  #Computing local angular windows for radial component
  windows_datasets = sapply(pred_phi,SPAR_empirical_windows,polar_data=polar_data,num_neigh=num_neigh,simplify = F)
  ...
  #return estimated threshold and parameter functions, along with the angular grid and non-exceedance probability. These are given in a list
  return(list(pred_thresh=emp_thresh,pred_para=emp_para,pred_phi=pred_phi,thresh_prob=thresh_prob))
}
...
#wrapper for obtaining empirical radial windows for each of the reference angles on the polar scale.
SPAR_empirical_windows_polar = function(ref_phi,polar_data,num_neigh){
  #ref_phi - reference angle 
  #polar_data - angular radial dataframe 
  #num_neigh - number of neighbours for each local angular windows
...
SPAR_simulation = function(nsim,SPAR_GPD,SPAR_ang){ #wrapper for simulating from smooth model fits
  #nsim - number of simulations to obtain from fitted SPAR model
  #SPAR_GPD - output from SPAR_smooth. Must be a list
  #SPAR_ang - output from SPAR_angular_density. Must be a vector
...
#Function for simulating from fitted SPAR model on the polar scale. This is only possible with the smooth model fits
SPAR_simulation_polar = function(nsim,SPAR_GPD,SPAR_ang){ #wrapper for simulating from smooth model fits
  #nsim - number of simulations to obtain from fitted SPAR model
  #SPAR_GPD - output from SPAR_smooth_polar. Must be a list
  #SPAR_ang - output from SPAR_angular_density_polar. Must be a vector
