...
fit_SPAR_model = function(sample_data,norm_choice="L1",thresh_prob,k,k_shape=NULL,pred_Q=seq(-2,2,length.out=201)){ #wrapper for fitting the SPAR model 
...
#Insert function for obtaining contours density estimate
SPAR_equidensity_contours = function(density_level,norm_choice="L1",SPAR_GPD,SPAR_ang){
  ...
  #norm_choice - this denotes the choice of norm. Must equal either "L1" or "L2". Defaults to "L1"
  #SPAR_GPD - output from either fit_SPAR_model or fit_SPAR_empirical. Must be a list
  ...
  if(density_level < 0 | length(density_level) != 1){
    stop("density_level should be a positive real number")
  }
  if(density_level > dens_lb){
    stop("density_level is too high, such that the SPAR model is not valid. Please try a smaller value")
  }
  if(!(norm_choice %in% c("L1","L2"))){
    stop("norm_choice must equal either 'L1' or 'L2'")
  }
  if(!is.list(SPAR_GPD)){
    stop("SPAR_GPD must be a list obtained as output from either fit_SPAR_model or fit_SPAR_empirical")
  }
  ...
  for(j in 1:length(pred_Q)){
    ...
    dens_radii[j] = uniroot(joint_dens_root_func,interval = c(SPAR_GPD$pred_thresh[j],50),u=SPAR_GPD$pred_thresh[j],scale=SPAR_GPD$pred_para$scale[j],shape=SPAR_GPD$pred_para$shape[j],thresh_prob=SPAR_GPD$thresh_prob,f_q=SPAR_ang[j],dens_val=density_level)$root
  }
  
  #Computing the corresponding contour in cartesian coordinates
  estimated_density_contour = cbind(dens_radii*u_vec,dens_radii*v_vec); estimated_density_contour = rbind(estimated_density_contour,estimated_density_contour[1,])
  
  #returning estimated density contour for normalised data. This is in the form of a matrix
  return(estimated_density_contour)
}
