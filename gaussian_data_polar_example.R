...
polar_smooth_fit = SPAR_smooth_polar(sample_data = example_data,thresh_prob = thresh_prob,k=k,pred_phi = pred_phi)

polar_local_fit = SPAR_local_polar(sample_data = example_data,thresh_prob = thresh_prob,pred_phi = pred_phi,num_neigh = num_neigh)

polar_angular_density = SPAR_angular_density_polar(sample_data = example_data,norm_choice = "L1",pred_Q = pred_Q,bw = bw)

density_levels = 10^(-(3:8)) #density levels for which to evaluate isodensity contours 

polar_isodensity_density_curves = SPAR_equidensity_contours_polar(density_levels = density_levels,norm_choice="L2",SPAR_GPD=L2_smooth_fit,SPAR_ang=L2_angular_density)
...
pdf(file="plots/gaussian_local_smooth_L1.pdf",width=12,height=4)
#Setting plotting parameters
par(mfrow=c(1,3),mgp=c(2.5,1,0),mar=c(5,4,4,2)+0.1)

#Comparing local and smooth estimates of threshold function
plot(pred_Q,L1_smooth_fit$pred_thresh,xlab="Q",ylab=expression(u[gamma]),main="Threshold",sub="Norm choice = L1",typ="l",col=2,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L1_smooth_fit$pred_thresh,L1_local_fit$pred_thresh))
lines(pred_Q,L1_local_fit$pred_thresh,lwd=4,col="red")

#Comparing local and smooth estimates of scale function
plot(pred_Q,L1_smooth_fit$pred_para$scale,xlab="Q",ylab=expression(tau),main="Scale",sub="Norm choice = L1",typ="l",col=3,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L1_smooth_fit$pred_para$scale,L1_local_fit$pred_para$scale))
lines(pred_Q,L1_local_fit$pred_para$scale,lwd=4,col="green")

#Comparing local and smooth estimates of shape function
plot(pred_Q,L1_smooth_fit$pred_para$shape,xlab="Q",ylab=expression(xi),main="Shape",sub="Norm choice = L1",typ="l",col=4,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L1_smooth_fit$pred_para$shape,L1_local_fit$pred_para$shape))
lines(pred_Q,L1_local_fit$pred_para$shape,lwd=4,col="blue")

dev.off()

pdf(file="plots/gaussian_local_smooth_L2.pdf",width=12,height=4)
#Setting plotting parameters
par(mfrow=c(1,3),mgp=c(2.5,1,0),mar=c(5,4,4,2)+0.1)

#Comparing local and smooth estimates of threshold function
plot(pred_Q,L2_smooth_fit$pred_thresh,xlab="Q",ylab=expression(u[gamma]),main="Threshold",sub="Norm choice = L2",typ="l",col=2,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L2_smooth_fit$pred_thresh,L2_local_fit$pred_thresh))
lines(pred_Q,L2_local_fit$pred_thresh,lwd=4,col="red")

#Comparing local and smooth estimates of scale function
plot(pred_Q,L2_smooth_fit$pred_para$scale,xlab="Q",ylab=expression(tau),main="Scale",sub="Norm choice = L2",typ="l",col=3,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L2_smooth_fit$pred_para$scale,L2_local_fit$pred_para$scale))
lines(pred_Q,L2_local_fit$pred_para$scale,lwd=4,col="green")

#Comparing local and smooth estimates of shape function
plot(pred_Q,L2_smooth_fit$pred_para$shape,xlab="Q",ylab=expression(xi),main="Shape",sub="Norm choice = L2",typ="l",col=4,cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3,lwd=4,ylim=range(L2_smooth_fit$pred_para$shape,L2_local_fit$pred_para$shape))
...
pdf(file="plots/gaussian_isodensity_contours.pdf",width=12,height=6)

#Setting plotting parameters
par(mfrow=c(1,2),mgp=c(2.5,1,0),mar=c(5,4,4,2)+0.1)

#Plots comparing estimated and true isodensity contours
plot(gaussian_density_contours[[1]],xlab="X",ylab="Y",main="Isodensity contours for Gaussian copula",sub="Norm choice = L1",type="l",col=1,lwd=3,xlim=range(gaussian_density_contours),ylim=range(gaussian_density_contours),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
for(i in 1:length(gaussian_density_contours)){
  ...
  lines(L1_isodensity_density_curves[[i]],lwd=3,col=colfunc_est(length(gaussian_density_contours))[i],lty=2)
}
...
plot(gaussian_density_contours[[1]],xlab="X",ylab="Y",main="Isodensity contours for Gaussian copula",sub="Norm choice = L2",type="l",col=1,lwd=3,xlim=range(gaussian_density_contours),ylim=range(gaussian_density_contours),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
for(i in 1:length(gaussian_density_contours)){
  lines(gaussian_density_contours[[i]],lwd=3,col=colfunc_true(length(gaussian_density_contours))[i])
  lines(L2_isodensity_density_curves[[i]],lwd=3,col=colfunc_est(length(gaussian_density_contours))[i],lty=3)
}
legend(-15,15,legend=paste0("10^(",-(1:length(gaussian_density_contours))-2,")"),lwd=3,col=colfunc_true(length(gaussian_density_contours)),cex=1.2,bg="white")
...
pdf(file="plots/gaussian_ang_dens.pdf",width=12,height=6)

#Setting plotting parameters
par(mfrow=c(1,2),mgp=c(2.5,1,0),mar=c(5,4,4,2)+0.1)

#Plots comparing angular density estimates to truth for both sets of estimates
plot(true_angdens_L1$q,true_angdens_L1$fq,type="l",lwd=3,col=2,xlab="q",ylab=expression(f[Q](q)),main="Angular density",sub="Norm choice = L1",ylim=range(L1_angular_density,true_angdens_L1$fq),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
lines(pred_Q,L1_angular_density,lwd=3,col=3)
legend(-2,1,legend=c("True","Estimated"),lwd=3,col=c(2,3),cex=1.2,bg="white")

#Plots comparing angular density estimates to truth for both sets of estimates
plot(true_angdens_L2$q,true_angdens_L2$fq,type="l",lwd=3,col=2,xlab="q",ylab=expression(f[Q](q)),main="Angular density",sub="Norm choice = L2",ylim=range(L2_angular_density,true_angdens_L2$fq),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
lines(pred_Q,L2_angular_density,lwd=3,col=3)
legend(-2,1,legend=c("True","Estimated"),lwd=3,col=c(2,3),cex=1.2,bg="white")

dev.off()

pdf(file="plots/gaussian_simulated_data.pdf",width=12,height=6)

#Setting plotting parameters
par(mfrow=c(1,2),mgp=c(2.5,1,0),mar=c(5,4,4,2)+0.1)

#Plotting simulated data over original sample with L1 breakdown 
plot(example_data,xlab="X",ylab="Y",main="SPAR model simulations",sub="Norm choice = L1",col="grey",pch=16,lwd=3,xlim=range(example_data,L1_simulation$data_sample),ylim=range(example_data,L1_simulation$data_sample),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
...
plot(example_data,xlab="X",ylab="Y",main="SPAR model simulations",sub="Norm choice = L2",col="grey",pch=16,lwd=3,xlim=range(example_data,L2_simulation$data_sample),ylim=range(example_data,L2_simulation$data_sample),cex.lab=1.2, cex.axis=1.2,cex.main=1.55,cex.sub=1.3)
